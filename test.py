...
def lab_attack(X_nat, c_trg, model, epsilon=0.05, iter=100, gamma=0.99, 
    ...
    for step in range(iter):
        ...
        epsilon = min(0.05, max(0.01, epsilon - 0.001 * reward))   
        # reward > 0: 보상이 높을수록 epsilon 감소 → 작은 섭동으로 정밀한 활용 수행.
        # reward < 0: 보상이 낮을수록 epsilon 증가 → 큰 섭동으로 더 많은 탐색 수행.
        ...
        # epsilon 범위 추적
        epsilon_min = torch.min(torch.tensor(perturb)).item()
        epsilon_max = torch.max(torch.tensor(perturb)).item()
        ...
        print(f"[Iteration {step}] Loss: {reward:.4f}, "
              ...
              f"Epsilon Range: {epsilon_min:.4f} to {epsilon_max:.4f}, "
